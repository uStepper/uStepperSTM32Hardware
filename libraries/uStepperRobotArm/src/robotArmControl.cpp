...
uint16_t valveCompareMatch = 107;
...
void robotArmControl::begin(uint8_t device) {
  ...
  if (bus.addressNum == BASE) {
    ...
    valveCompareMatch = (uint16_t)((5.0 * 4096.0)/supplyVoltage);
	pumpCompareMatch = (uint16_t)((12.0 * 4096.0)/supplyVoltage);
	if (pumpCompareMatch > 4095)
	{
		pumpCompareMatch = 4095;
	}
	if (valveCompareMatch > 4095)
	{
...
void robotArmControl::slaveLoop() 
{
	...
	while (1)
	{
		...
		else if (state == runContinouslyCmd)
		{
			stepper.setMaxVelocity(currentCommandArgument.f);
