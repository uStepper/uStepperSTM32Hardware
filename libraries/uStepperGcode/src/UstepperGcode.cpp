...
UstepperGcode::UstepperGcode(void) {
	this->commandCount = 0;
...
void UstepperGcode::setSendFunc(void (*func)(char *data)){
	this->sendFunc = func;
}

void UstepperGcode::addCommand( char * command, void (*func)(char *command, char *data) ){
...
void UstepperGcode::printCommands( void ){
...
void UstepperGcode::enableCRC(bool state){
	this->useCRC = state;
}

bool UstepperGcode::check(char *cmd ) {
	return this->check( cmd, this->packet );
}

bool UstepperGcode::check(char *cmd, char *packet ) {
	char buf[10] = {'\0'};
	uint8_t len = 0;
...
char *UstepperGcode::getPacket(void) {
	return this->packet;
}

uint8_t UstepperGcode::getStatus(void){
	return this->status;
}

bool UstepperGcode::value(char *name, float *var) {
	// If no packet is supplied, work on the latests in buffer
	return this->value(name, var, this->packet);
}

bool UstepperGcode::value(char *name, int32_t *var) {
	// If no packet is supplied, work on the latests in buffer
	return this->value(name, var, this->packet);
}

bool UstepperGcode::value(char *name, int32_t *var, char *packet ){
	float temp = 0.0;
...
bool UstepperGcode::value(char *name, float *var, char *packet ) {
	char *start;
	char *end;
	size_t len;

	char buf[20] = {'\0'};
	start = strstr(packet, name);
...
void UstepperGcode::insert(char data){
...
uint8_t UstepperGcode::process(void){
...
/* 
uint8_t UstepperGcode::read(void) {
...
void UstepperGcode::send(char *command) { 
	this->send(command, this->useCRC);
}

void UstepperGcode::send(char *command, bool checksum)
{
	char buf[MAX_PACKET_SIZE] = {'\0'};
	strcpy(buf, command);
